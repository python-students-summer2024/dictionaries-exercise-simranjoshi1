...
def display_cookies(cookies):
    ...
    for cookie in cookies:
        print("#{} - {}".format(cookie['id'], cookie['title']))
...
def solicit_quantity(id, cookies):
    ...
    if cookie:
        while True:
            try:
                quantity = int(input("My favorite! How many {} would you like? ".format(cookie['title'])))
                if quantity > 0:
                    subtotal = quantity * cookie['price']
                    print("Your subtotal for {} {} is ${:.2f}.\n".format(quantity, cookie['title'], subtotal))
                    return quantity
                else:
...
def display_order_total(order, cookies):
    ...
    for item in order:
        ...
        if cookie:
            ...
            print("- {} {}".format(item['quantity'], cookie['title']))
    print("\nYour total is ${:.2f}.".format(total_cost))
